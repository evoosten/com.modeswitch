...
class ModeSwitchApp extends Homey.App {
  async onInit() {
    ...
    this.modeChangedTrigger = this.homey.flow.getTriggerCard('mode_changed');
    ...
    this.log('Mode Switch app v2.7.4 is ready');
  }
  ...
  getModeLabel(mode) {
    ...
    return this.homey.__(`mode.${mode}`) || mode;
  }
  ...
  async applyMode(mode, { source = 'manual' } = {}) {
    ...
    for (const deviceId of selectedRules.on) {
      try {
        await this._setDeviceCapabilityValue(deviceId, 'onoff', true);
    ...
    for (const deviceId of selectedRules.off) {
      try {
        await this._setDeviceCapabilityValue(deviceId, 'onoff', false);
    ...
    await this.modeChangedTrigger.trigger({ mode, previous_mode: previousMode, source });
...
const isDay = hour >= 7 && hour < 18;
  ...
  async getSwitchableDevices() {
    return (await this.getEnvironment()).switchableDevices;
  }
  ...
  async applyTemperatureRulesForCurrentMode({ source = 'manual' } = {}) {
    ...
    const rules = this.getTemperatureRules().filter(rule => rule.enabled && rule.modeId === currentMode);
  ...
  _getSleepSignals(environment, settings) {
    ...
    const powerById = byId(environment.powerDevices);
  ...
  async _applyDeviceAction(deviceIds, value) {
    for (const deviceId of this._sanitizeDeviceIdList(deviceIds)) {
      try {
        await this._setDeviceCapabilityValue(deviceId, 'onoff', value);
      } catch (error) {
  ...
  _capabilitiesForSwitchEvent(event, device) {
    ...
    if (event === 'on' || event === 'off' || event === 'toggle') return capabilities.includes('onoff') ? ['onoff'] : [];
    if (event === 'dim_changed' || event === 'dim_up' || event === 'dim_down') return capabilities.includes('dim') ? ['dim'] : [];
  ...
  _matchesSwitchCapabilityEvent(event, capabilityId, value, current, previous) {
    if (event === 'on') return capabilityId === 'onoff' && value === true;
    if (event === 'off') return capabilityId === 'onoff' && value === false;
    if (event === 'toggle') return capabilityId === 'onoff';
    if (event === 'dim_changed') return capabilityId === 'dim';
    if (event === 'dim_up') return capabilityId === 'dim' && typeof value === 'number' && typeof previous.dim === 'number' && value > previous.dim;
    if (event === 'dim_down') return capabilityId === 'dim' && typeof value === 'number' && typeof previous.dim === 'number' && value < previous.dim;
  ...
  _isSwitchInputDevice(device) {
    ...
    if (['remote', 'button', 'scene', 'other'].includes(cls) && caps.some(cap => String(cap).startsWith('button') || String(cap).includes('scene'))) {
      return true;
    }
    ...
    if ((caps.includes('onoff') || caps.includes('dim')) && remoteHints.some(hint => driverId.includes(hint) || name.includes(hint))) {
      return true;
    }
  ...
  _getSwitchInputSnapshot(device) {
    ...
    for (const capability of capabilities) {
      const capId = String(capability);
      if (capId === 'onoff' || capId === 'dim' || capId.startsWith('button') || capId.includes('scene')) {
  ...
  async _toggleDeviceAction(deviceIds) {
    const environment = await this.getEnvironment();
    const byId = new Map((environment.switchableDevices || []).map(device => [device.id, device]));
    await Promise.all(this._sanitizeDeviceIdList(deviceIds).map(async deviceId => {
      const current = byId.get(deviceId)?.onoff === true;
      try { await this._setDeviceCapabilityValue(deviceId, 'onoff', !current); }
      catch (error) { this.error('Toggle action failed for ' + deviceId, error); }
    }));
  }
  ...
  async _applyDimAction(deviceIds, level) {
    ...
    await Promise.all(this._sanitizeDeviceIdList(deviceIds).map(async deviceId => {
      try {
        await this._setDeviceCapabilityValue(deviceId, 'onoff', true);
        await this._setDeviceCapabilityValue(deviceId, 'dim', dim);
      } catch (error) {
        this.error('Dim action failed for ' + deviceId, error);
      }
    }));
  }
  ...
  async _pollApplianceRules() {
    ...
    try {
      ...
      const deviceById = new Map((environment.powerDevices || []).map(device => [device.id, device]));
      const state = this.homey.settings.get(SETTINGS_KEYS.APPLIANCE_STATE) || {};
      let changed = false;
      const now = Date.now();
      ...
      for (const rule of rules) {
        try {
          ...
          if (!device || typeof device.measurePower !== 'number') continue;
          ...
          if (current.status === 'running') {
            ...
            if (deltaMs > 0 && Number.isFinite(previousPower)) {
  ...
  async _evaluateSubModeAutoRules(subModeRules, environment, now) {
    ...
    const powerById = new Map((environment.powerDevices || []).map(device => [device.id, device]));
    const context = {
  ...
  _defaultOperatorForActivityType(type) {
    ...
    if (type === 'sensor_true' || type === 'device_on' || type === 'activity_status') return 'true';
  ...
  async _pollActivityRules(rules, environment, now) {
    ...
    const devices = new Map(((environment.activityDevices || [])
      .concat(environment.powerDevices || [], environment.waterDevices || [], environment.gasDevices || [], environment.temperatureSensorDevices || [], environment.humiditySensorDevices || []))
    ...
    for (const rule of rules) {
      try {
        ...
        const rawConditionResults = conditions.map(condition => {
          try {
            return this._evaluateActivityCondition(condition, devices, current, now, state, rule.id);
          } catch (error) {
        ...
        const running = trueCount >= threshold;
  ...
  _evaluateActivityCondition(condition, deviceById, activityState = null, now = Date.now(), allActivityState = null, currentActivityId = '') {
  ...
  _getActivityMeterSnapshot(rule, deviceById) {
    ...
    const addDevice = deviceId => {
      ...
      if (!device) return;
      snapshot[deviceId] = {
        meter_power: Number(device.capabilityValues?.meter_power),
  ...
  _updateActivityUsage(current, deviceById, now) {
    ...
    for (const [deviceId, device] of deviceById.entries()) {
      ...
      const values = device.capabilityValues || {};
      if (Number.isFinite(Number(values.meter_power)) && Number.isFinite(Number(baselines[deviceId]?.meter_power))) {
        current.energyKwh = Math.max(Number(current.energyKwh || 0), Number(values.meter_power) - Number(baselines[deviceId].meter_power));
      } else if (Number.isFinite(Number(values.measure_power)) && deltaMs > 0) {
        current.energyKwh = Number(current.energyKwh || 0) + Math.max(0, Number(values.measure_power)) * deltaMs / 3600000000;
      }
  ...
  _hasApplianceZoneActivity(rule, environment) {
    const appliance = (environment.powerDevices || []).find(device => device.id === rule.deviceId)
  ...
  _normalizeTemperatureRule(rule) {
    ...
    const modeId = this.getAvailableModes().includes(rule.modeId) ? rule.modeId : 'home';
    const zoneId = typeof rule.zoneId === 'string' ? rule.zoneId : '';
